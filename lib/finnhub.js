...
const DEFAULT_FINNHUB_TOKEN = 'd4maeh1r01qjidhujfmgd4maeh1r01qjidhujfn0';

function withToken(path, params = {}) {
  const url = new URL(`${FINNHUB_BASE}${path}`);
  const token = process.env.FINNHUB_API_KEY || DEFAULT_FINNHUB_TOKEN;
...
export async function getCompanyNews(symbol) {
  try {
    ...
    if (primary.ok) {
      const companyNews = await primary.json();
      if (Array.isArray(companyNews) && companyNews.length > 0) return companyNews;
    }

    const general = await fetch(withToken('/news', { category: 'general' }), { next: { revalidate: 60 } });
    if (!general.ok) throw new Error('General news request failed');
    return general.json();
  } catch (error) {
